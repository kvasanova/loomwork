...
## Claude Code is both the host and the target

This repository is a Claude Code plugin, so the session editing it may also be running it. Changes to `hooks/hooks.json` or `hooks/*.sh` take effect only after the plugin reloads — a gate that misbehaves mid-session is usually the old copy still loaded, not a bug in the edit.

## Path variables

- `${CLAUDE_PLUGIN_ROOT}` — the plugin's own root. Skills invoke the CLIs through it: `node "${CLAUDE_PLUGIN_ROOT}/scripts/sdd-audit.mjs"`. Never hardcode a path to `scripts/` in skill or command Markdown.
...
- `strategy-gate.sh` exits silently when `CLAUDE_PROJECT_DIR` is unset, so the gate never nudges from an unrelated directory. Keep that guard.
